...
현재 버전은 `0.7.3`입니다. 버전별 작업 내역과 기록 규칙은 [`CHANGELOG.md`](CHANGELOG.md)에서 확인할 수 있습니다.
...
엔진은 게임 문서의 `runtime` 직렬화와 복원 규격을 선택 기능으로 지원합니다. 현재 단판형 프로토타입 데이터셋은 `persistence`를 선언하지 않으므로 진행 자동저장과 이어하기 UI를 사용하지 않으며, 최고 점수표만 로컬스토리지에 유지합니다.
