...
function bindControls():void{
  ...
  window.addEventListener('keydown',event=>{const advance=event.code==='KeyZ'||event.code==='Space'||event.code==='Enter';if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'].includes(event.code))event.preventDefault();if(event.repeat||!advance)return;if(dialogueAdvance){event.stopImmediatePropagation();dialogueAdvance();return;}if(!$('#result-screen').classList.contains('hidden')){event.stopImmediatePropagation();quickRetry();return;}if(!$('#title-screen').classList.contains('hidden')){event.stopImmediatePropagation();startGame();}});
